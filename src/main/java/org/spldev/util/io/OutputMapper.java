...
public abstract class OutputMapper extends IOMapper<Output> {
    ...
    abstract protected Output createOutput(Path path) throws IOException;
    ...
    public static class Stream extends OutputMapper {
        ...
        public Stream(OutputStream outputStream, Charset charset) {
            super(Map.of(DEFAULT_MAIN_PATH, new Output.Stream(outputStream, charset)),
                    DEFAULT_MAIN_PATH);
        }
    ...
    public static class String extends OutputMapper {
        ...
        public Map<Path, java.lang.String> getOutputStrings() {
            return ioMap.entrySet().stream()
                    .collect(Collectors.toMap(Map.Entry::getKey, e -> e.getValue().getOutputStream().toString()));
        }
    }
    ...
    public static OutputMapper of(Path mainPath, Charset charset, Options... options) throws IOException {
        return Arrays.asList(options).contains(Options.OUTPUT_FILE_JAR)
                ? new JARFile(
                IOObject.getPathWithExtension(mainPath, "jar"),
                mainPath.getFileName(),
                charset)
                : Arrays.asList(options).contains(Options.OUTPUT_FILE_ZIP)
                ? new ZIPFile(
                IOObject.getPathWithExtension(mainPath, "zip"),
                mainPath.getFileName(),
                charset)
    ...
    public void withMainPath(Path newMainPath, IORunnable ioRunnable) throws IOException { // todo: handle relative paths / subdirs?
        create(newMainPath);
