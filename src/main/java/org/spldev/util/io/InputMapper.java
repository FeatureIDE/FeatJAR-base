...
public abstract class InputMapper extends IOMapper<Input> {
    ...
    public static class Stream extends InputMapper {
        public Stream(Map<Path, InputStream> pathInputStreamMap, Path rootPath, Path mainPath,
                      Charset charset, java.lang.String fileExtension) {
            ...
            for (Path currentPath : pathInputStreamMap.keySet()) {
                ioMap.put(relativizeRootPath(rootPath, currentPath),
    ...
    public static class File extends InputMapper {
        ...
        public File(Path mainPath, Charset charset, Options... options) throws IOException {
            this(Arrays.asList(options).contains(Options.INPUT_FILE_HIERARCHY)
                    ? getFilePathsInDirectory(mainPath.getParent())
                    : List.of(mainPath),
                    mainPath.getParent(), mainPath, charset);
        }
    }

    public static class String extends InputMapper {
        public String(Map<Path, java.lang.String> pathStringMap, Path rootPath, Path mainPath,
                      Charset charset, java.lang.String fileExtension) {
            super(relativizeRootPath(rootPath, mainPath));
    ...
    public <T> Result<T> withMainPath(Path newMainPath, Supplier<Result<T>> supplier) { // todo: handle relative paths / subdirs?
